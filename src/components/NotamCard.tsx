...
interface NotamCardProps {
  ...
  isSelected?: boolean;
  onSelect?: (notamId: string) => void;
}

const NotamCard: React.FC<NotamCardProps> = ({ notam, onMarkAsRead, isSelected, onSelect }) => {
  ...
  return (
    <div
      className={`notam-card ${notam.shouldHighlight ? 'highlight' : ''} ${notam.isRead ? 'read' : 'unread'} ${isSelected ? 'selected' : ''}`}
      ...
      <div className="notam-header">
        ...
        <div className="notam-badges">
          {notam.shouldHighlight && notam.wasReadWhenFar && (
            <span className="badge badge-warning">NOW CLOSE</span>
          )}
          {notam.shouldHighlight && notam.resurfaced && (
      ...
      <div className="notam-details">
        ...
        {notam.readAt && (
          <div className="detail-row">
            ...
            <span className="detail-value">
              {formatDate(notam.readAt)}
              {notam.daysSinceRead !== undefined && ` (${notam.daysSinceRead}d ago)`}
            </span>
          </div>
        )}
        {notam.wasReadWhenFar && (
          <div className="detail-row warning-row">
            ...
            <span className="detail-value warning-text">
              Was read when far from route, now close
            </span>
          </div>
        )}
        {notam.resurfaced && (
