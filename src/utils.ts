...
// Thresholds
export const CLOSE_THRESHOLD_NM = 30;
export const FAR_THRESHOLD_NM = 100;
export const RESURFACE_DAYS = 7;
...
export function processNotamsWithStatus(
  ...
  return notams.map(notam => {
    ...
    let daysSinceRead: number | undefined;

    if (isRead && readStatus) {
      const distanceWhenRead = readStatus.distanceFromRoute;
      daysSinceRead = daysSince(readStatus.readAt);

      // Was read when far (>100 NM) but now close (<30 NM) - proximity alert
      if (distanceWhenRead > FAR_THRESHOLD_NM && currentDistance < CLOSE_THRESHOLD_NM) {
        shouldHighlight = true;
        wasReadWhenFar = true;
      }

      // Resurface if read more than 7 days ago
      if (daysSinceRead >= RESURFACE_DAYS) {
        shouldHighlight = true;
        resurfaced = true;
      }
    } else {
